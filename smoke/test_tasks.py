...
def test_list_tasks(base_url, workspace_id):
    # Action
    response = httpx.get(f"{base_url}/ws/{workspace_id}/tasks/")
    # Expected
...
def test_filter_tasks_by_status(base_url, workspace_id):
    # Action
    response = httpx.get(f"{base_url}/ws/{workspace_id}/tasks/?status=pending")
    # Expected
    data = response.json()
    assert response.status_code == 200
...
def test_pagination_moves_window(base_url, workspace_id):
    # Action
    response = httpx.get(f"{base_url}/ws/{workspace_id}/tasks/?page_size=5&page=2")
    # Expected
    data = response.json()
    assert response.status_code == 200
...
def test_get_task_by_id(base_url, workspace_id):
    # Action
    response = httpx.get(f"{base_url}/ws/{workspace_id}/tasks/1")
    # Expected
    expected = {
        "id": 1,
...
def test_get_task_not_found(base_url, workspace_id):
    # Action
    response = httpx.get(f"{base_url}/ws/{workspace_id}/tasks/999999")
    # Expected
...
def test_create_task(base_url, workspace_id):
    ...
    # Action
    response = httpx.post(f"{base_url}/ws/{workspace_id}/tasks/", json=payload)
    # Expected
    expected = {
        "id": ANY,
        "created_at": ANY,
...
def test_patch_task(base_url, workspace_id):
    # Setup
    created = httpx.post(
        f"{base_url}/ws/{workspace_id}/tasks/",
        json={"title": "Patch Target", "description": "Original", "status": "pending"},
    ).json()
    task_id = created["id"]
    # Action
    response = httpx.patch(f"{base_url}/ws/{workspace_id}/tasks/{task_id}", json={"status": "in_progress"})
    # Expected
    expected = {
        "id": task_id,
        "created_at": ANY,
...
def test_delete_task(base_url, workspace_id):
    ...
    task_id = created["id"]
    # Action
    response = httpx.delete(f"{base_url}/ws/{workspace_id}/tasks/{task_id}")
    # Expected
